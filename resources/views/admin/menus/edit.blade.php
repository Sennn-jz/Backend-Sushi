...
<div class="mb-8">
    ...
    <p class="text-slate-500 pl-8">Update your restaurant's menu details.</p>
</div>
    ...
    <form action="{{ route('admin.menus.update', $menu) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="col-span-1 md:col-span-2">
                ...
                <input type="text" name="name" value="{{ old('name', $menu->name) }}" required class="block w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:bg-white focus:ring-1 focus:ring-orange-500 focus:border-orange-500 transition-colors">
            ...
            <div>
                ...
                <select name="category" required class="block w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:bg-white focus:ring-1 focus:ring-orange-500 focus:border-orange-500 transition-colors">
                    ...
                    <option value="Sashimi" {{ old('category', $menu->category) == 'Sashimi' ? 'selected' : '' }}>Sashimi</option>
            ...
            <div>
                ...
                <input type="number" name="price" value="{{ old('price', $menu->price) }}" required min="0" class="block w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:bg-white focus:ring-1 focus:ring-orange-500 focus:border-orange-500 transition-colors">
                @error('price') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
            </div>

            <div class="col-span-1 md:col-span-2">
                <label class="block text-sm font-medium text-slate-700 mb-2">Description</label>
                <textarea name="description" rows="3" required class="block w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:bg-white focus:ring-1 focus:ring-orange-500 focus:border-orange-500 transition-colors">{{ old('description', $menu->description) }}</textarea>
            ...
            <div class="col-span-1 md:col-span-2">
                ...
                <div class="mb-4">
                    ...
                    <img src="{{ str_starts_with($menu->image_url, 'http') ? $menu->image_url : asset('storage/' . $menu->image_url) }}" alt="{{ $menu->name }}" class="h-32 rounded-lg border border-gray-200 object-cover">
                </div>
                @endif
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer" onclick="document.getElementById('file-upload').click()">
                    <div class="space-y-1 text-center">
                        ...
                        <p class="text-xs text-gray-500">PNG, JPG, WEBP up to 2MB. Leave empty to keep current image.</p>
                    </div>
                </div>
            ...
            <div class="col-span-1 md:col-span-2 flex items-center">
                <input id="is_available" name="is_available" type="checkbox" {{ old('is_available', $menu->is_available) ? 'checked' : '' }} class="h-4 w-4 text-orange-500 focus:ring-orange-500 border-gray-300 rounded">
